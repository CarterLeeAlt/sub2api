      ...
      >
        <div class="mb-3 account-setting-row flex items-center justify-between">
          ...
          <button
            ...
            role="switch"
            :aria-checked="accountSchedulingThresholdOverrideEnabled"
            @click="accountSchedulingThresholdOverrideEnabled = !accountSchedulingThresholdOverrideEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              accountSchedulingThresholdOverrideEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                accountSchedulingThresholdOverrideEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    if (isCNProviderPlatform(newAccount.platform) || newAccount.platform === 'opencode_go') {
      if (newAccount.platform === 'opencode_go') {
        ...
        // DeepSeek 没有 Coding Plan；旧数据即使带有 coding 也必须在编辑/保存路径归一化。
        editAccountMode.value =
          newAccount.platform === 'deepseek' ? 'payg' : credentials.account_mode === 'coding' ? 'coding' : 'payg'
      }
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      ...
      if (isCNApiKeyAccount.value) {
        // DeepSeek 没有 Coding Plan；旧数据即使带有 coding 也必须在保存路径归一化。
        newCredentials.account_mode =
          props.account?.platform === 'deepseek' ? 'payg' : currentOpenCodeOrCNMode()
