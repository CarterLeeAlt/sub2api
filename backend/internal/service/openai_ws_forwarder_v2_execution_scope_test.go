...
func TestOpenAIGatewayService_Forward_WSv2_TurnStateBoundToExecutionScope(t *testing.T) {
	...
	_, turnStateOnLegacy := store.GetSessionTurnState(groupID, legacyHash, account.ID)
	require.False(t, turnStateOnLegacy, "turn state 不得绑定到按 session_id 算出的会话哈希")
	turnState, turnStateOnScope := store.GetSessionTurnState(groupID, scope, account.ID)
...
func TestOpenAIGatewayService_Forward_WSv2_ExecutionScopeUsesOriginalIdentity(t *testing.T) {
	...
	_, boundA := stateStore.GetSessionTurnState(groupID, scopeA, account.ID)
	require.True(t, boundA, "会话 A 的 turn state 应落在按原始 session_id 算出的作用域")
	_, boundB := stateStore.GetSessionTurnState(groupID, scopeB, account.ID)
	...
	_, boundToInjected := stateStore.GetSessionTurnState(groupID, injectedScope, account.ID)
	...
	_, boundC := stateStore.GetSessionTurnState(groupID, scopeC, account.ID)
