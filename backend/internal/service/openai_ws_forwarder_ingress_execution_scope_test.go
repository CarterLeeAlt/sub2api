...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StateBoundToExecutionScope(t *testing.T) {
	...
	_, turnStateOnLegacy := stateStore.GetSessionTurnState(groupID, legacyHash, account.ID)
	require.False(t, turnStateOnLegacy, "turn state 不得绑定到会话哈希")
	turnState, turnStateOnScope := stateStore.GetSessionTurnState(groupID, scope, account.ID)
