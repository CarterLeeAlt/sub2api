...
- 分页扫描主账号，单账号只执行一次完整 `QueryUsage`，由同一次上游检查同时刷新 5h/7d WHAM 额度和重置卡信息；**串行逐账号查询**（避免并发检查触发上游失败），单账号超时 20s 只覆盖查询本身，失败后等待 10 秒用新超时重试一次，仍失败保留上次成功缓存；串行后整轮时长随账号数线性增长，刷新周期在轮末计算、自然顺延；
- global WHAM 窗口写入主账号，`codex_bengalfox` 窗口写入关联 Spark 影子账号；重置卡只读快照分发到父子行并与额度快照使用同一观测时间，影子账号仍禁止消费；
- `extra.codex_reset_credit_snapshot` 增加可选 RFC3339Nano `fetched_at`，仓储使用独立的严格更新代际 CAS，拒绝相同或更旧响应覆盖；旧快照无该字段时继续兼容并在下次成功刷新时升级；
- 上游只返回重置卡数量时照常缓存数量和采集时间，不伪造到期时间；前端从 `extra` 水合，同账号快照变化也重新水合。只有 30 分钟内的新鲜快照或成功实时查询明确返回 `available_count` 时才显示次数（包括具有业务含义的 `0`）；字段缺失、陈旧、查询失败或回读失败时隐藏旧次数和到期明细，并明确显示最近检查时间与过期状态；
...
合并提交：[`8acf06e2b`](https://github.com/CarterLeeAlt/sub2api/commit/8acf06e2b834cf7c5c1eafb8fa235e7d6c352745)。
